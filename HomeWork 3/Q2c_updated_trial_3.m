...
% I=(image1)>93;
bw = bwareaopen(I,20);
% BW2 = bwmorph3(BW2,'clean');figure(8);imshow(uint8(255*I1));
se = strel('disk',1);
bw = imclose(bw,se);
bw = imfill(bw,'holes');
% imshow(bw)
...
for i=2:height+1
    for j=2:width+1
            G(i,j)=S_unconditional(F(i,j),M(i,j),M(i,j+1),M(i-1,j+1),M(i-1,j),M(i-1,j-1),M(i,j-1),M(i+1,j-1),M(i+1,j),M(i+1,j+1));
    end
end
figure(4);
imshow(G);
