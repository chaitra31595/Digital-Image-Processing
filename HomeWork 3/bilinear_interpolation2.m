function G=bilinear_interpolation2(image,PQ_pos,height,width)
% Boundary_extended_image=255*ones(N+2,N+2);
% Boundary_extended_image(2:N+1,2:N+1)=image;
% image=Boundary_extended_image;

if((mod(PQ_pos(1,1),1)==0) && (mod(PQ_pos(2,1),1)==0) && (PQ_pos(1,1)>=1)&&(PQ_pos(1,1)<=height) && (PQ_pos(2,1)>=1)&& (PQ_pos(2,1)<=width))
...
elseif((mod(PQ_pos(1,1),1)==0) && (mod(PQ_pos(2,1),1)~=0))
    ...
    b=double([fQ11 fQ12]);
%     d=double((1/((ceil(PQ_pos(2,1))-floor(PQ_pos(2,1))))));
    G=b*a;
...
%     d=double((1/((ceil(PQ_pos(1,1))-floor(PQ_pos(1,1))))));
    G=c*b;    
