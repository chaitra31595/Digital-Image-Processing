...
Boundary_extended_image(1,2:width+1)=image(2,:);
Boundary_extended_image(height+2,2:width+1)=image(height-1,:);
Boundary_extended_image(1:height+2,1)=Boundary_extended_image(:,2);
Boundary_extended_image(1:height+2,width+2)=Boundary_extended_image(:,width-1);
...
for i=2:height+1
    for j=2:width+1
        ...
        if (F(i,j)==0)
           F2(i,j)=connectivity_4(F(i,j),F(i,j+1),F(i-1,j),F(i,j-1),F(i+1,j));
           if(F2(i,j)==1)
              F2(i,j)=connectivity_8(F(i,j),F(i,j+1),F(i-1,j+1),F(i-1,j),F(i-1,j-1),F(i,j-1),F(i+1,j-1),F(i+1,j),F(i+1,j+1));
           end
        end
    end
end
